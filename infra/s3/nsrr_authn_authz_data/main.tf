terraform {
 ...
 backend "s3" {
    ...
    key    = "prod/s3/nsrr_authn_authz_data"
    region = "us-east-1"
  }
}
...
locals {
  bucket_name = "nsrr-authn-authz-data"
}
...
resource "aws_s3_bucket_public_access_block" "data_block_public_access"{
...
resource "aws_s3_bucket_policy" "sslpolicy" {
  bucket = aws_s3_bucket.data_bucket.id
